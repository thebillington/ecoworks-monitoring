...
export default function AttendanceSheetComponent(
    props: IAttendanceSheetProps
) {
    const attendees = props.attendanceSheet.attendees
    return (
        <>
                    ...
                    <tbody>
                        {
                            attendees.map( ( email, i ) => 
                                <tr key={ email } className={`bg-white ${i != attendees.length - 1 ? 'border-b ' : ' '}dark:bg-gray-800 dark:border-gray-700`}>
