...
export default function AttendanceCheckboxComponent( props: IAttendanceCheckboxProps) {
    ...
    return (
        <label className="flex items-center cursor-pointer relative">
            <input type="checkbox" checked={checked} onChange={ (e) => toggleAttendance(e, props.user.email) } checked={checked} className="peer h-5 w-5 cursor-pointer transition-all appearance-none rounded shadow hover:shadow-md border border-slate-300 checked:bg-green-600 checked:border-green-600" />
